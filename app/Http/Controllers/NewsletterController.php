...
class NewsletterController extends Controller
{
    ...
    {
        if ($request->search) {
            $subscribers = Newslettersubscriber::orderBy('id', 'desc')->where('email', 'like', "%$request->search%")->paginate(20);
        } else {
            $subscribers = Newslettersubscriber::orderBy('id', 'desc')->paginate(20);
        }
    ...
    public function send_newsletters(Request $request)
    {
        ...
        if ($request->send_to == 'all') {
            ...
            $members = Newslettersubscriber::get();
        } elseif ($request->send_to == 'registered_subscriber') {
            $members = Newslettersubscriber::join('users', 'newsletter_subscribers.email', '=', 'users.email')->get();

    ...
    public function index(Request $request, $category = '')
    {
        ...
        if ($category != '') {
            ...
            if ($category_details->parent_id == 0) {
                ...
                $query      = $query->whereIn('courses.category_id', $sub_cat_id);
            } else {
                $page_data['parent_cat'] = Category::where('id', $category_details->parent_id)->value('slug');
                $query                   = $query->where('courses.category_id', $category_details->id);
            }
        }

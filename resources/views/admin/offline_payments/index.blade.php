                                                        ...
                                                        @if ($payment->item_type == 'course')
                                                            @foreach (App\Models\Course::whereIn('id', json_decode($payment->items, true))->get() as $course)
                                                                <p class="sub-title text-12px">
                                                                    <a href="{{ route('course.details', Str::slug($course->title)) }}" class="text-muted me-3">{{ $course->title }} </a>
                                                                </p>
                                                            @endforeach
                                                        @endif

                                                        @if ($payment->item_type == 'bootcamp')
                                                            @foreach (App\Models\Bootcamp::where('id', $payment->items)->get() as $bootcamp)
                                                                <p class="sub-title text-12px">
                                                                    <a href="{{ route('bootcamp.details', ['slug' => Str::slug($bootcamp->title)]) }}" class="text-muted me-3">{{ $bootcamp->title }} </a>
                                                                </p>
                                                            @endforeach
                                                        @endif

                                                        @if ($payment->item_type == 'package')
                                                            @foreach (App\Models\TeamTrainingPackage::where('id', $payment->items)->get() as $package)
                                                                <p class="sub-title text-12px">

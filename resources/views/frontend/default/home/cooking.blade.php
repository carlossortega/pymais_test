...
@section('content')
            ...
            <div class="row row-28">
                <div class="col-lg-6 order-2 order-lg-1">
                    ...
                    <div class="cooking-banner-details">
                        <p class="light">{{ get_phrase('WELLCOME TO CHEF') }}</p>
                        <h1 class="title mb-20"><span class="highlight">{{ $first_word }}</span> <span class="small">{{ $second_word }}</span> {{ $remaining_text }}</h1>
                        <p class="info mb-30">{{ get_frontend_settings('banner_sub_title') }}</p>

                        <div class="d-flex align-items-center">
                            <a href="{{ route('courses') }}" class="rectangle-btn1">{{ get_phrase('Visit Courses') }}</a>
                            <a class="rectangle-btn2 ms-5" data-bs-toggle="modal" data-bs-target="#promoVideo" href="#"><i class="fa-solid fa-play"></i>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="cooking-section-title">
                        <h3 class="title mb-30">{{ get_phrase('Top Rated Courses') }}</h3>
                        <p class="info">
                            {{ get_phrase("Lorem Ipsum has been the industry's standard dummy text ever since the  unknown printer took a galley of type and scrambled") }}</p>
                    </div>
                </div>
            </div>
            <div class="row row-28 mb-110">
                @php
                    $top_courses = DB::table('courses')->join('reviews', 'courses.id', '=', 'reviews.course_id')->select('courses.*', DB::raw('COUNT(reviews.id) as review_count'), DB::raw('AVG(reviews.rating) as average_rating'))->groupBy('courses.id')->orderByDesc('review_count')->take(4)->get();
                @endphp
                                ...
                                <div class="cooking-course-card-body">
                                    ...
                                    <div class="author-price d-flex align-items-center justify-content-between flex-wrap">
                                        ...
                                        @if (isset($row->discount_flag) && $row->discount_flag == 1)
                                            <p class="price">{{ currency(number_format($row->price - $row->discounted_price, 2)) }}</p>
    ...
    <section>
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <div class="cooking-section-title">
                        <h3 class="title mb-30">{{ get_phrase('Upcoming Courses') }}</h3>
                        <p class="info">
                            {{ get_phrase("Lorem Ipsum has been the industry's standard dummy text ever since the  unknown printer took a galley of type and scrambled") }}</p>
                    </div>
                </div>
    ...
    <section>
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <div class="cooking-section-title">
                        <h3 class="title mb-30">{{ get_phrase('Featured Courses') }}</h3>
                        <p class="info">
                            {{ get_phrase("Lorem Ipsum has been the industry's standard dummy text ever since the  unknown printer took a galley of type and scrambled") }}</p>
                    </div>
                </div>
                ...
                <div class="col-md-12">
                    ...
                    <div class="cooking-counter-area d-flex">
                        <div class="cooking-counter-single">
                            <h2 class="total"><span class="counter1">{{ count($total_students) }}</span>+</h2>
                            <p class="info">{{ get_phrase('Enrolled Learners') }}</p>
                        </div>
                        <div class="cooking-counter-single">
                            <h2 class="total"><span class="counter1">{{ count($total_instructors) }}</span></h2>
                            <p class="info">{{ get_phrase('Online Instructors') }}</p>
                        </div>
                        <div class="cooking-counter-single">
                            <h2 class="total"><span class="counter1">{{ count($premium_courses) }}</span>+</h2>
                            <p class="info">{{ get_phrase('Premium courses') }}</p>
                        </div>
                        <div class="cooking-counter-single">
                            <h2 class="total"><span class="counter1">{{ count($free_courses) }}</span>+</h2>
                        ...
                        <div class="become-instructor-area d-flex align-items-center justify-content-between">
                            <div class="become-instructor-video-area">
                                @if (isset($instructor_speech->image))
                                    <img src="{{ asset('assets/upload/home_page_image/cooking/' . $instructor_speech->image) }}" alt="">
                                @else
                                @endif
                            ...
                            <div class="become-instructor-details">
                                <h3 class="title mb-30">{{ $instructor_speech->title }}</h3>
    ...
    <section>
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <div class="cooking-section-title">
                        <h3 class="title mb-30">{{ get_phrase('Our Popular Instructor') }}</h3>
                        <p class="info">
                            {{ get_phrase("Lorem Ipsum has been the industry's standard dummy text ever since the  unknown printer took a galley of type and scrambled") }}</p>
                    </div>
                </div>
                ...
                @foreach ($popular_instaructors as $key => $instructor)
                    ...
                    <div class="col-lg-4 col-md-6 col-sm-6">
                        <div class="cooking-popular-instructor">
                            ...
                            <div class="details">
                                <a href="{{ route('instructor.details', ['name' => Str::slug($instructorDetails->name), 'id' => $instructorDetails->id]) }}" class="w-100">
                                    <h5 class="name">{{ $instructorDetails->name }}</h5>
    ...
    <section>
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <div class="frequently-qna-title-area mb-50 d-flex align-items-center justify-content-between">
                        <h1 class="title">{{ get_phrase('Frequently Asked Questions?') }}</h1>
                        <p class="info">
                        ...
                        <div class="accordion qnaaccordion-two" id="accordionExampleRight">
                            @foreach ($even_faqs as $key => $faq)
                                <div class="accordion-item">
                                    ...
                                    <div id="collapseRight{{ $key }}" class="accordion-collapse collapse {{ $key == 0 ? 'show' : '' }}" data-bs-parent="#accordionExampleRight">
                                        <div class="accordion-body">
                                            <p class="answer">{{ $faq['answer'] }}</p>
                                        </div>
                                    </div>
            ...
            <div class="container">
                <div class="cooking-news-main-area">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="cooking-section-title">
                                <h3 class="title mb-30">{{ get_phrase('Follow The Latest News') }}</h3>
                                <p class="info">
                                ...
                                <a href="{{ route('blog.details', $blog->slug) }}" class="list-news1-link">
                                    <div class="list-link1-card d-flex align-items-center">
                                        ...
                                        <div class="list-news1-card-body">
                                            ...
                                            <p class="info mb-0 ellipsis-line-4">{{ ellipsis(strip_tags($blog->description), 160) }}</p>
